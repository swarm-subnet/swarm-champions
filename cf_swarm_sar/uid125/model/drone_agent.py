...
class Posterior:
    ...
    def _recompute(self) -> None:
        ...
        if POST_ON and _POST_HEAT['h'] is not None and self.clue_xy is not None:
            _u = np.clip((sx - self.clue_xy[0] + POST_HALF) / POST_CELL, 0, POST_G - 1).astype(np.int64)
            _v = np.clip((sy - self.clue_xy[1] + POST_HALF) / POST_CELL, 0, POST_G - 1).astype(np.int64)
            _hh = _POST_HEAT['h'][_v, _u]
            _hm = float(_hh.max())
            if _hm > 1e-12:
                w = w * ((1.0 - POST_ALPHA) + POST_ALPHA * (_hh / _hm))
        tot = w.sum()
...
# v230: forest bar reverted to the king's default. Measured as churn, not gain:
# panel W/L 23/24 on 152 forest seeds, live 7 failures vs the king's 6 (-0.0282).
COMMIT_CONF_FOREST = float(os.environ.get('SAR220_FOREST', '0.60'))
...
R_HOVER_M = 3.0
CONFIRM_SPEED = float(os.environ.get('SAR182_CONFSPD', '0.9')) / SPEED_LIMIT
SPIRAL_R0_M = 4.0
SPIRAL_RMAX_M = 26.0
...
MTN_CLIMB_GAIN = 1.2
MTN_STEER_DAMP = 0.4
# v440 learned victim-location posterior over the clue-anchored 45x45 grid.
POST_ON = os.environ.get('SAR500_POST', '0') == '1'
POST_ALPHA = float(os.environ.get('SAR500_ALPHA', '0.6'))
POST_TEMP = float(os.environ.get('SAR500_TEMP', '1.0'))
POST_GATE = int(os.environ.get('SAR500_GATE', '1500'))
POST_G = 45
POST_HALF = 90.0
POST_CELL = 2.0 * POST_HALF / POST_G
_POST_HEAT = {'h': None}
_POST_RT = {'sess': None, 'mu': None, 'sd': None, 'dead': False}


def _post_heat_compute(state, depth):
    if _POST_RT['dead']:
        return None
    try:
        if _POST_RT['sess'] is None:
            import onnxruntime as _ort
            _d = os.path.dirname(os.path.abspath(__file__))
            _o = _ort.SessionOptions(); _o.intra_op_num_threads = 1; _o.inter_op_num_threads = 1
            _POST_RT['sess'] = _ort.InferenceSession(os.path.join(_d, 'post_net.onnx'), _o,
                                                     providers=['CPUExecutionProvider'])
            _nz = np.load(os.path.join(_d, 'post_norm.npz'))
            _POST_RT['mu'] = _nz['mu'].astype(np.float32); _POST_RT['sd'] = _nz['sd'].astype(np.float32)
        n = int(state.shape[0])
        st = np.zeros((1, 8, 214), np.float32)
        dp = np.zeros((1, 8, 32, 32), np.float32)
        mk = np.zeros((1, 8), bool)
        take = min(n, 8)
        st[0, :take] = (np.round(state[:take, :214], 4) - _POST_RT['mu']) / _POST_RT['sd']
        d64 = np.round(depth[:take].reshape(take, 64, 4, 64, 4).mean(axis=(2, 4)), 2)
        d32 = d64.reshape(take, 32, 2, 32, 2).mean(axis=(2, 4))
        dp[0, :take] = d32 / max(1e-6, float(d64.max()))
        mk[0, :take] = True
        lg = _POST_RT['sess'].run(['logits'], {'state': st, 'depth': dp, 'mask': mk})[0][0]
        lg = lg.astype(np.float64) / max(1e-6, POST_TEMP)
        lg -= lg.max()
        h = np.exp(lg); h /= h.sum()
        return h.reshape(POST_G, POST_G)
    except Exception:
        _POST_RT['dead'] = True
        return None
...
FOREST_WH_VETO = 0.25
FOREST_SPEED = float(os.environ.get('SAR_FOREST_SPEED', '1.0'))
RAYBRAKE_M = float(os.environ.get('SAR_RAYBRAKE_M', '0.0'))
RAYBRAKE_SPEED = float(os.environ.get('SAR_RAYBRAKE_SPEED', '0.35'))
RAYLIFT_BIAS = float(os.environ.get('SAR_RAYLIFT_BIAS', '0.0'))
AVOID_SLOW_WIDE_M = float(os.environ.get('SAR_AVOID_SLOW_WIDE', '0.0'))
WIDE_ROWHI = int(os.environ.get('SAR_WIDE_ROWHI', '160'))
...
URBAN_OVERRIDE_GATE_M = float(os.environ.get('SAR_URBAN_OVERRIDE_GATE', '1.05'))
URBAN_OVERRIDE_P = float(os.environ.get('SAR_URBAN_OVERRIDE_P', '2.0'))
...
YAWGATE_DEG = float(os.environ.get('SAR350_YAWGATE', '35'))   # v360: gate ON by default (validated 2x out-of-sample)
YAWGATE_SPEED = float(os.environ.get('SAR350_YAWSPEED', '0.35'))
# v351: generalise the validated gate along its two axes. Defaults reproduce
# v350 exactly; each extension is a separate env knob so arms stay one-delta.
YAWGATE_MAPS = tuple(x for x in os.environ.get('SAR351_MAPS', 'forest').split(',') if x)
    ...
    def steer(self, direction: np.ndarray, speed: float, depth: np.ndarray, rpy: np.ndarray) -> tuple[np.ndarray, float]:
        ...
        if getattr(self, 'wide', False):
            brake_ref = (self._narrow_centre(depth)
                         if os.environ.get('SAR700_BRAKE_NARROW', '1') == '1'
        ...
        if nearest <= AVOID_LOOKAHEAD_M:
            ...
            if float(push @ push) > float(self.mem_bias @ self.mem_bias):
                self.mem_bias = push
            _slow_m = AVOID_SLOW_WIDE_M if (AVOID_SLOW_WIDE_M > 0.0 and getattr(self, 'wide', False)) else AVOID_SLOW_M
            self.mem_slow = min(self.mem_slow, float(np.clip(brake_ref / _slow_m, 0.25, 1.0)))
...
class _V21Controller:
    ...
    def _hover_policy(self, vic, head_z, pos):
        ...
        if self._sweep_t0 is not None:
            ...
            if ph <= _d0:
                dz = 2.5 - ph
            elif ph <= _d0 + (HOVSWEEP_HI - HOVSWEEP_LO):
    ...
    def act(self, observation) -> np.ndarray:
        ...
        self.avoider.wide = bool(getattr(self, 'forest_low', False)) \
            ...
            and (os.environ.get('SAR710_FOREST_ONLY', '1') != '1'
        ...
        _gate = _mapsw_gate()
        if _gate > URBAN_OVERRIDE_GATE_M and URBAN_OVERRIDE_P <= 1.0:
            try:
                if (self.mtn.prob_of('city') + self.mtn.prob_of('village')) >= URBAN_OVERRIDE_P:
                    _gate = URBAN_OVERRIDE_GATE_M
            except Exception:
                pass
        elevated = not self.mtn.is_mountain and vic[2] - ground_z > _gate and (float(np.linalg.norm(vic[0:2] - pos[0:2])) > INVESTIGATE_M)
        ...
        if HOVSWEEP and self.mtn.is_mountain and self.frozen:
            if self._sweep_t0 is None and self.hover_stable_ticks >= HOVSWEEP_TRIG:
                self._sweep_t0 = self.tick
            if self._sweep_t0 is not None:
                _giveup_n = HOVSWEEP_GIVEUP
        if self.hover_stable_ticks >= _giveup_n or self.hover_total_ticks >= HOVER_GIVEUP_FALLBACK_TICKS:
        ...
        # v350 TURN-THEN-GO (forest): the env flies world-frame velocity setpoints
        # instantly while commanded yaw is chased through yaw dynamics, so at every
        # sweep turn and avoider side-push, motion leads the nose. Measured on 15
        # instrumented crash seeds: median yaw-vs-velocity misalignment 73 deg in
        # the last 0.5 s before impact, 94% beyond the camera's 45 deg half-FOV --
        # the avoider cannot see the tree it is being pushed into. Gate: while the
        # nose is more than YAWGATE_DEG off the commanded track in forest search,
        # cap speed so the drone turns before it translates. Straight legs are
        # untouched, so this cannot starve detection the way a blanket slowdown
        # (SAR_FOREST_SPEED=0.6: -0.0372, +5 failures) did.
        if (YAWGATE_DEG > 0.0 and _MAPSW_STATE.get("map") in YAWGATE_MAPS
            ...
            if _mis > np.radians(YAWGATE_DEG):
                speed = min(speed, YAWGATE_SPEED)
        if (RAYBRAKE_M > 0.0 and self.forest_low and self.mode != 'hover'
                and self._last_agl is not None and self._last_agl < RAYBRAKE_M):
            speed = min(speed, RAYBRAKE_SPEED)
            if RAYLIFT_BIAS > 0.0:
                direction = _unit(direction + np.array([0.0, 0.0, RAYLIFT_BIAS]))
        if self.mode != 'hover':
            if self.mtn.is_mountain:
...
class _TeamState:
    ...
    def guard_confirmer_band(self, out, pos, agl, live, beliefs=None):
        ...
        a = float(agl[i])
        changed = False
        # v473 band-hold servo: within SAR730_H of the candidate, stop relying on
        # the one-sided envelopes and servo the drone to band centre directly --
        # the 8-seed trace census showed arrivals hang above the band, punch
        # through its floor, or transit it too fast; nothing holds them there.
        if (os.environ.get('SAR730_HOLD', '1') == '1'
                and _MAPSW_STATE.get("map") in tuple(
                    x for x in os.environ.get('SAR730_MAPS', 'city').split(',') if x)
                and h <= float(os.environ.get('SAR730_H', '1.5'))
                and self.cand is not None):
            # v477: anchor the height servo on AGL, not est z -- the 713 trace
            # showed a ~2m-low victim-z estimate steering a perfect hold to
            # 1.2m above the victim, below the band floor. AGL 3.8 puts height-
            # above-top at ~2.9-3.5 for ground victims regardless of est error.
            agl_t = float(os.environ.get('SAR730_AGLT', '3.8'))
            z_err = agl_t - a
            # v476 state gate: only steer when the drone is NOT already in a
            # confirmable state (band height, slow). In-band-and-slow means the
            # original confirm machinery is working -- hands off, or we break
            # the confirms that already succeed (v473 broke 4 on 5FKk).
            _spd_now = float(np.linalg.norm(_vel_of(out[i])))
            if (abs(z_err) <= float(os.environ.get('SAR730_ZBAND', '0.7'))
                    and _spd_now <= 0.9):
                z_err = None
            if z_err is not None:
                vz = max(-0.55, min(0.45, 0.6 * z_err))
                hx = 0.8 * (float(ref_xy[0]) - float(pos[i][0]))
                hy = 0.8 * (float(ref_xy[1]) - float(pos[i][1]))
                hn = math.hypot(hx, hy)
                if hn > 0.35:
                    hx *= 0.35 / hn
                    hy *= 0.35 / hn
                v = np.array([hx, hy, vz], dtype=np.float64)
                s = float(np.linalg.norm(v))
                if s > 0.9:
                    v *= 0.9 / s
                _store_vel(out[i], v)
                self.stats["band_hold"] = self.stats.get("band_hold", 0) + 1
                return out
...
class TeamLayer:
    ...
    def _act_inner(self, obs, state, n, t0):
        ...
        if POST_ON and _POST_HEAT.get('pend') is None and _POST_HEAT['h'] is None:
            _hp = _post_heat_compute(state, depth)
            _POST_HEAT['pend'] = _hp if _hp is not None else False
        if (POST_ON and _POST_HEAT['h'] is None
                and _POST_HEAT.get('pend') is not None and _POST_HEAT.get('pend') is not False
                and T.tick >= POST_GATE
                and not any(self._lock_of(_i) > 0.0 for _i in range(n))):
            _POST_HEAT['h'] = _POST_HEAT['pend']
            if getattr(T, 'region', None) is not None:
                try:
                    T.region.set_type_belief(T.p_env_team)
                except Exception:
                    pass
...
_np = np  # map-switch alias


# ============================ MAP SWITCH (added) ============================
# Per-map ground gate, decided once at tick 15 from the depth statistics
# the agent already computes. See the build script for the full rationale.
#
# uid159's own XGBoost map classifier cannot gate this: it scores 52.5% on fresh
# seeds because it is a cf_autopilot model fed a 214-wide row it was never
# trained on. This layer scores 95.5% IN FLIGHT (open 95.7%, n=800) using ONLY
# _map_depth_feature_values, so it costs one 68x5 matmul and no XGBoost call.
...
# Module-level because the gate is read deep inside the per-drone controller and
# every drone in an episode shares one policy instance and one map. Reset clears
# it, so a stale label cannot leak into the next episode in a reused worker.
_MAPSW_STATE = {"map": None}


def _mapsw_gate():
    """Ground gate for the CURRENT episode's map.

    Falls back to the shipped value whenever the map is unknown -- before tick
    15, or if classification fails for any reason -- so the failure
    mode is uid159's stock behaviour, never something new. city and forest map to
    the stock value by profile, so a misclassification INTO either of them is
    also just stock behaviour."""
    return _MAPSW_GATE_BY_MAP.get(_MAPSW_STATE["map"], _MAPSW_BASE_GATE)
...
def _mapsw_classify(obs):
    try:
        ...
        # Mean over drones, matching how the layer was fitted: features were
        # averaged across drones at the same tick, so averaging here rather than
        # voting keeps deployment identical to the fit.
        f = _np.mean([_np.asarray(_map_depth_feature_values(dp[i]), _np.float64)
                      for i in range(n)], axis=0)
        if f.shape[0] != _MAPSW_MU.shape[0]:
            return None
        z = (f - _MAPSW_MU) / _MAPSW_SD
        return _MAPSW_MAPS[int(_np.argmax(z @ _MAPSW_W + _MAPSW_B))]
    except Exception:
        return None
...
class DroneFlightController(_MapswBase):
    """uid159 with the map decided once per episode.

    Subclasses rather than wraps so that anything the harness reaches for on the
    base class -- attributes, properties, the team layer -- keeps working
    untouched; only act/reset are extended.
    """
    ...
    def act(self, observation):
        ...
        # Latch ONCE. Switching a control constant mid-episode has been measured
        # in this family to produce a third outcome worse than either setting.
...
# r55: uid 1 carries a village skip for the consensus residual but ties it to an
# unrelated flag -- the default is 'village' only when SAR710_FOREST_ONLY is OFF,
# and they ship it ON, so the skip never engages and the residual runs on village
# too. It costs them: their village is 0.8866 against the champion's 0.9350, the
# weakest village among the leading builds, while our champion-based r48 beats
# them there by +0.0076 head to head. The r9 note in our own lineage measured the
# same thing from the other side: consensus is worth +0.0251 mountain, +0.0162
# city, +0.0123 open, +0.0018 forest -- and -0.0118 village on 0 wins in 7.
# Untie the flags and let the skip do what it was written to do. This is the
# merge in one line: uid 1's open and forest, the champion's village.
...
class DroneFlightController(_UID66_TERMINAL_BASE):
    def act(self, observation):
        ...
        try:
            ...
            return action
# === end frozen public terminal-set residual ===
