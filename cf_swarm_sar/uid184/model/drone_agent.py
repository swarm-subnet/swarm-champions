...
class _V21Controller:
    ...
    def act(self, observation) -> np.ndarray:
        ...
        if self.hover_stable_ticks >= _giveup_n or self.hover_total_ticks >= HOVER_GIVEUP_FALLBACK_TICKS:
            ...
            self._sweep_t0 = None
        if self.mode == 'initial':
            _deck = self._search_alt()
            if pos[2] >= ground_z + _deck - 1.0 or self.tick > 150:
                self.mode = 'search'
        elif self.locked:
...
class _TeamState:
    ...
    def update_candidate(self, beliefs, pos, live):
        ...
        if self.confirmer >= 0 and self._has_cand():
            if TEAM_SUPERSEDE and self.mtn and self.n_supersede < TEAM_SUPERSEDE_MAX:
        ...
        for i in live:
            ...
            want = {
                ...
                "deny_xy": None if (T.cand is None or i == T.confirmer
                                    or (TEAM_DENY_FIX and T.mtn and T.confirmer < 0)) else T.cand["xy"],
