...
class ActorVisibleGoalPosterior:
    """Expose ``score`` expected by UID134's remaining-mass planner.

    The neural prediction is computed once from the reset observation.  Later
    pad detections and traversed rays are discounted by UID134's existing
    planner, so the network never receives simulator-only goal state.
    """
