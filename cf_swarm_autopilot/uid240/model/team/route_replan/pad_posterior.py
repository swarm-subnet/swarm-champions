...
def pad_field(starts, clue, kind, step=2.0, nR=25, found=(), W=None, radial=None):
    ...
    if radial is not None and kind in radial:
        F = [empirical_prior(gx, gy, starts[j], *_radial_for_start(radial, kind, starts[j], W), W) for j in open_idx]
    else:
...
def pad_field_found(starts, clue, kind, found_xy, step=4.0, nR=25, W=None, radial=None):
    ...
    gy = gx.copy()
    if radial is not None and kind in radial:
        F = [empirical_prior(gx, gy, S[j], *_radial_for_start(radial, kind, S[j], W), W) for j in range(n)]
    else:
