...
class SwarmAutopilotAgent:
    ...
    def _merge(self, xyz: np.ndarray, rng=None, view=None, obs=None) -> None:
    ...
    def _assign_par(self, state: np.ndarray, cand, free, spoken) -> bool:
        ...
        for a, b in zip(rows, cols):
            ...
            if self.pads[k].by is not None or self.pads[k].done:
                continue
            xy = self.pads[k].xyz[:2]
            if any(float(np.linalg.norm(xy - sp)) < self.cfg.same_pad for sp in spoken):
                continue
            self.d[i].claim = k
            self.pads[k].by = i
            self.d[i].phase = APPROACH
    ...
    def _replan_tick(self, state) -> None:
        ...
        try:
            ...
            pl = SearchPlanner(K=K, layouts=int(cfg.search_replan_layouts), pop=int(cfg.search_replan_pop),
                               ...
                               climb_delay=climb, land_delay=land, speed=spd)
        ...
        if dd.phase == CLIMB:
            ...
            if cfg.land_settle:
                if dd.hop:
                    ...
                    if dist <= cfg.land_settle_r:
                        v_des[0] = 0.0
                        v_des[1] = 0.0
                    elif dist <= (cfg.forest_abort_radius if self.is_forest else cfg.abort_radius) \
