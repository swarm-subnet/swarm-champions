...
class SwarmAutopilotAgent:
    ...
    def _free_dir(self, depth: np.ndarray, i: int, v: np.ndarray,
        ...
        if _cg_ext:
            _ex = FD.extrude_pts(depth[i], R, float(pos[2]) + 0.05,
                                 ...
                                 dz=tuple(c.free_extrude_dz), vox=float(c.free_extrude_vox),
        ...
        out = world * speed * (1.0 if (agree > 0.98 and not _guard) else c.free_slow)
    ...
    def _dead_zero(self, state: np.ndarray) -> None:
        ...
        for i in range(self.n):
            ...
            kept = None
            if dd.claim is not None:
                if float(np.linalg.norm(np.asarray(self.pads[dd.claim].xyz, float)[:2] - xy)) <= float(c.landed_r):
                    kept = int(dd.claim)
                else:
    ...
    def _ne_tick(self, live, poses, rots, depth, h0) -> None:
        ...
        for k, p in enumerate(self.pads):
            if k >= len(h0) or int(p.hits) > int(h0[k]):
                ...
                continue
            if p.done or p.ne_safe or int(p.hits) < 1 or int(p.hits) >= int(c.ne_max_hits):
                continue
            if p.by is not None:
                j = int(p.by)
    ...
    def act(self, observation) -> np.ndarray:
        ...
        if self.step_i % cfg.detect_every == 0:
            ...
            if live and det is not None:
                ...
                _h0 = [int(p.hits) for p in self.pads] if _ne else None
                _gla = self._gla_on()
