...
from .geo_posterior import (MAP_PARAMS, _sample_goals_one, assignment_weights, band_match,
                            clue_log_likelihood, sample_goals_banded)

SPEED = 3.0
DT = 0.5
T_MAX = 58.0
DEFAULTS = dict(det_range=22.0, sector_deg=95.0, climb_delay=0.5, land_delay=3.0, speed=SPEED, blind_r=0.0)
...
def sample_layouts(starts_xy: np.ndarray, clue_xy: np.ndarray, kind: str, L: int,
    ...
    g, _bk, _mix = sample_goals_banded(starts_xy, kind, M, rng)     # one shared band per joint draw
...
def sample_layouts_conditioned(starts_xy: np.ndarray, clue_xy: np.ndarray, kind: str, found_xy: np.ndarray,
    ...
    g, bk, mix = sample_goals_banded(S, kind, M, rng)                         # (n, M, 2), shared band per draw
    band_w = np.array([w for _, _, w in mix])
    kk = min(F, n)
    perms = np.array(list(itertools.permutations(range(n), kk)), dtype=np.int64) if n <= 8 else None
    if perms is None or len(perms) == 0:
        return None, None
    Lb = band_match(g, bk, len(band_w), found[:kk], match_sigma)
    ...
    for a, cnt in zip(order, counts):
        ...
        w = np.exp(clue_log_likelihood(cen, np.asarray(clue_xy, np.float32)[:2])) * band_lik[bk]
        w = np.nan_to_num(w, nan=0.0, posinf=0.0, neginf=0.0)
        if w.sum() <= 0:
            w = np.ones(M)
...
def simulate(plans: np.ndarray, starts_xy: np.ndarray, layouts: np.ndarray, tt: np.ndarray,
             ...
             pad_found0: Optional[np.ndarray] = None, blind_r: float = 0.0) -> np.ndarray:
    ...
    while t < T_MAX:
        ...
        # the camera looks forward with a +-45 deg vertical field of view, so ground closer
        # (horizontally) than the drone's height above it is below the frame and unseen
        p_r = np.where(dist >= blind_r, p_r, 0.0)
...
class SearchPlanner:
    ...
    def start(self, starts_xy, clue_xy, kind: str, init_plan: Optional[np.ndarray] = None):
        ...
        self._kw = dict(det_range=self.params["det_range"], sector_deg=self.params["sector_deg"],
                        ...
                        speed=float(self.params.get("speed", SPEED)), blind_r=float(self.params.get("blind_r", 0.0)))
    ...
    def start_conditioned_phase(self):
        ...
        self._kw = dict(det_range=self.params["det_range"], sector_deg=self.params["sector_deg"],
                        ...
                        speed=float(self.params.get("speed", SPEED)), t0=a["t0"], blind_r=float(self.params.get("blind_r", 0.0)))
        init = a["init_plan"]
        if init.shape[1] >= self.K:
            mean = init[:, : self.K, :].copy()
        else:
    ...
    def start_conditioned(self, cur_xy, orig_free_xy, orig_starts_xy, clue_xy, kind: str, found_xy,
        ...
        self._kw = dict(det_range=self.params["det_range"], sector_deg=self.params["sector_deg"],
                        climb_delay=0.0, land_delay=self.params["land_delay"], world=W,
                        speed=float(self.params.get("speed", SPEED)), t0=float(t0), blind_r=float(self.params.get("blind_r", 0.0)))
        init = np.asarray(init_plan, np.float32)[:, :, :2]
        if init.shape[1] >= self.K:
            mean = init[:, : self.K, :].copy()
        else:
    ...
    def step(self, budget_sec: float = 0.4, chunk: int = 4) -> int:
        ...
        done_iters = 0
        _fixed = int(os.environ.get("SWARM_CEM_FIXED", "0"))   # DEV: >0 = exactly this many chunks per call
        _chunks = 0
        ...
        while not self.done:
            ...
            if _fixed > 0:
                if _chunks >= _fixed:
                    break
            elif time.perf_counter() - t0 >= budget_sec:
                break
        return done_iters
