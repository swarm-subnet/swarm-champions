...
class AutopilotConfig:
    ...
    commit_t_skip: tuple = ("forest", "village")
...
\
        ...
        yaw = (getattr(self, '_att_yaw_now', None) if (c.steer_true_frame and getattr(self, '_att_yaw_now', None) is not None)
               else self._yaw_now)
...
\
    ...
    def _one(self, state: np.ndarray, depth: np.ndarray, i: int) -> np.ndarray:
        ...
        if dd.phase == CLIMB:
            ...
            # APPROACH clipped vertical to +-1.0 m/s while DESCEND sinks at
            # 2.8 -- same airframe, and APPROACH never goes below pad_z+4 m.
            # When the horizontal leg is short the cap binds and the drone
            # waits on its own descent (APPROACH is 8.0 s, p90 11.6-14.6 s).
