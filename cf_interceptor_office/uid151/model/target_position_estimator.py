...
IMAGE_SIZE_PX = 256.0
TARGET_WIDTH_M = float(os.environ.get("OFFICE_TARGET_WIDTH_PRIOR_M", "0.18"))
TARGET_HEIGHT_M = float(os.environ.get("OFFICE_TARGET_HEIGHT_PRIOR_M", "0.08"))
# Compatibility-only legacy consensus model.  A paired intervention showed
# that correcting this internal constant without retraining the jointly
# calibrated filter creates a new collision.  The primary TargetTrack above
# uses the physical 0.035 m extrinsic; this isolated module keeps its learned
# coordinate convention until it is replaced atomically.
CAMERA_FORWARD_M = float(os.environ.get("OFFICE_CLOSE_CAMERA_FORWARD_M", "0.13"))
CAMERA_UP_M = float(os.environ.get("OFFICE_CAMERA_UP_M", "0.05"))
