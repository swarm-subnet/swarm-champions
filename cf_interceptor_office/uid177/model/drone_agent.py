...
# A submission is unpacked into its own directory and its sibling modules
# (office_pilot, posevis) are imported by bare name, which only resolves if the
# loader put that directory on sys.path. The validator does; `swarm model verify`
# does not, and neither did a plain import elsewhere -- which is the
# "No module named 'office_pilot'" this package failed verification with. The
# posevis import sits in a try/except, so there it does not even raise: it
# silently drops the localiser, the whole value of this build. Pin our own
# directory so both resolve wherever this is loaded from.
_bootstrap_sys.path.insert(0, _bootstrap_os.path.dirname(_bootstrap_os.path.abspath(__file__)))
...
class FeatureBuilder:
    def __init__(self, grid=None, loc_particles: int = 4096, seed: int = 0):
        ...
        try:
            ...
            self._vp = VisualPose(str(_PHERE / "posenet16.pt"), threads=1)
        except Exception:
            self._vp = None
    ...
    def build(self, state: np.ndarray, rgb=None) -> np.ndarray:
        ...
        if self._vp is not None and rgb is not None and ego.pos[2] >= 0.15:
            try:
