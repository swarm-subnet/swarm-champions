...
class Cfg:
    def __init__(self) -> None:
        ...
        self.eg_r = _f("OP_EG_R", 1.0)
        self.eg_dz = _f("OP_EG_DZ", 0.11)
        self.eg_k = _f("OP_EG_K", 8.0)
        self.eg_lim = _f("OP_EG_LIM", 0.60)
        self.eg_age = _f("OP_EG_AGE", 0.4)
        self.eg_up = _i("OP_EG_UP", 1)
        self.eg_zmax = _f("OP_EG_ZMAX", 2.80)
        self.eg_taper = _f("OP_EG_TAPER", 0.0)
        self.eg_hold = _f("OP_EG_HOLD", 0.0)
        ...
        self.sb_v = _f("OP_SB_V", 0.30)
        self.sb_t = _f("OP_SB_T", 2.0)
        self.sb_minr = _f("OP_SB_MINR", 1.2)
        self.sb_age = _f("OP_SB_AGE", 0.6)
        self.sb_speed = _f("OP_SB_SPEED", 2.0)
        self.sb_look = _f("OP_SB_LOOK", 3.0)
        self.sb_infl = _f("OP_SB_INFL", 0.30)
        self.sb_rec = _f("OP_SB_REC", 0.6)
        self.sb_yaw = _f("OP_SB_YAW", 0.0)
        self.sb_t0 = _f("OP_SB_T0", 3.0)
...
class Pilot:
    ...
    def _sticks(self, vx: float, vy: float, yaw: float):
        ...
        sp = self._spd
        return (s * vx - c * vy) / sp, (c * vx + s * vy) / sp

    def _vel(self, lr: float, fb: float, yaw: float):
        c, s = math.cos(yaw), math.sin(yaw)
        sp = self._spd
        return sp * (fb * c + lr * s), sp * (fb * s - lr * c)
    ...
    def step(self, a, fb):
        ...
        self._spd = float(getattr(ego, 'speed_hat', SPEED))
        t = float(ego.t)
        z = float(ego.pos[2])
        ...
        if self._term:
            ...
            if c.tm_vz:
                a[2] = float(np.clip(
                    (rel[2] + vt[2] * c.tm_lead) * c.tm_kz / self._spd, -1.0, 1.0))
            if c.tm_yawmode:
                az = math.atan2(
        ...
        if c.eg_on and rng is not None and track.rel is not None:
            ...
            if act:
                ...
                err = relz + dz
                u = float(np.clip(c.eg_k * err / self._spd, -c.eg_lim, c.eg_lim))
                if z > c.eg_zmax:
                    u = min(u, 0.0)
