...
DEFAULT_FLAGS = "slowmv,mvfit,mvpatience,invest,nospin,inward_mtn,mdesc,mreacq,keepview,brake,yawvel,gndfilt,gentle2,apptube,roofguard,fastapp,vsearch3,mhigharr,mspin2,vmovko,vsurf,vclamp,vbad1,vmem,vland,vcheck,vking,vretry,mdirect,mland,cmem,ccheck,cland,cretry,caround,cfast,cnoclimb,qretry,mking,cdetour,czcap,cstable,cmovko,clook,cskim,cwall,cdrift,ftake,fdown,fspin,wleash,wsearch,wspin,vdip,vabs"
...
# c22: camera yaw on the village search rings follows the flight heading (no inward bias) by default.
# uid183 hard-codes a 0.7 rad inward bias there. c19yh: +0.0083 flown (t=1.98, 11 rescued / 3 broken) over
# 4 tuning lists and +0.0063 (4/1) over 2 held-out epoch-21 lists. MY_VYAW=in restores uid183.
_VYAW = os.environ.get("MY_VYAW", "head")
...
class _MyController:
    ...
    def act(self, observation):
        ...
        if self.mode == "TAKEOFF":
            ...
            elif self.map_label == "mountain" and float(center[2]) > pos[2] + 2.0:
                # c11m - uphill route: climb to the goal's height early instead of hugging the rising
                # slope, where the forward depth image is all terrain and the planner brakes the cruise
                # to a crawl. Measured +0.0029 board (t=4.67, 9 lists) on uid128, which had the same
                # cruise; uid183 still lacks it and its mountain fell to 0.882.
            ...
            if self._track_reliable():
                ...
                if self.search_wps is None:
                    ...
                    for r in radii:
                        for k in range(n_wp):
                            ...
                            wps.append(wp_)
                    self._vdip_active = False
                    if (_flag("vdip") and self.map_label == "village" and anchor is None
                ...
                if not self.spin_done:
                    ...
                    if (self.spin_acc > spin_need and (not vdip or alt_c <= _VDIP_ALT_M + 0.6)) or (self.t - self.mode_t0) > spin_cap:
                        self.spin_done = True
                        self._vdip_active = False
                else:
...
_OPEN_ALT = float(os.environ.get("MY_OPEN_ALT", "12.0"))
    ...
    def _act_inner(self, observation):
                        ...
                        if kf is not None:
                            try:
                                arrived = (kf._mode == "search" and not bool(kf._goal_return_mode)
                                           ...
                                           and m_.center0 is not None and float(np.linalg.norm(pos_[:2] - m_.center0[:2])) < 3.0)
                            except Exception:
                                arrived = False
                        if arrived:
                            self._ws_done = True
        ...
        if self.route == "dual":
            ...
            if (a_king is not None and (self.mine.pad_ever_moving or self.mine.pad_moving) and fast_ok
                    and not getattr(self, "_vk_recaptured", False)):
                self.route = "king"
                if (_flag("vking") and self.mine.map_label == "village") or (_flag("mking") and self.mine.map_label == "mountain"):
