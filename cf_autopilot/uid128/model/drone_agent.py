...
# Behaviour switches. The packaged agent runs with DEFAULT_FLAGS; MY_SEARCH overrides them for A/B runs.
DEFAULT_FLAGS = "slowmv,mvfit,mvpatience,invest,nospin,inward_mtn,mdesc,mreacq,keepview,brake,yawvel,gndfilt,gentle2,apptube,roofguard,fastapp,vsearch3,mhigharr,mspin2"
...
class _MyController:
    ...
    def _update_track(self, dets, pos):
        ...
        if _flag("mmodel") and self.t - self.mm_t_fit >= 1.0:
            ...
            if self.mm is not None and not self.pad_moving and self.mm["span"] * self.mm["w"] >= 2.5:
                # a clean oscillation with >0.8 m amplitude over a good part of a period: it moves
                self.pad_moving = True
                self.pad_ever_moving = True
        ...
        if len(obs) >= 8 and (obs[-1][0] - obs[0][0]) >= 0.5 * win:
            ...
            if len(a) >= (12 if _flag("mvfit") else 20) and self.pad_moving:
                try:
                    ...
                    if _flag("mvfit") and z_mean < 10.0:
                        # velocity at the window end: removes the half-window lag that ruins reversals
                        te = float(tt[-1])
            ...
            if self.map_label in ("warehouse", "forest"):
                ...
                if _flag("slowmv"):
                    need_votes, need_sp, need_span = 6, 0.3, 0.4  # circular pads move as slowly as 0.36 m/s
            elif z_mean < 10.0:
    ...
    def _mm_fit(self):
        """Fit p(t) = c + A cos(wt) + B sin(wt) + C cos(2wt) + D sin(2wt) per axis over a grid of w.
        Covers the validator's circular / linear / figure-8 platform motions."""
        ...
        for w in np.arange(0.15, 0.67, 0.015):
            ...
            XtX = X.T @ X + np.diag([0.0, 0.0, 0.0, 2.0, 2.0])  # ridge on the 2w terms
            try:
    ...
    def _motion_at(self, dt):
        ...
        if abs(om) < 0.2 or sp < 0.1:
            if _flag("mvfit") and sp > 0.6:
                # fast straight-line pads reverse sinusoidally: carry the fitted acceleration for a short
                # horizon only (slow pads: the fitted acceleration is noise, extrapolating it kills the drop)
                da = min(dt, 1.5)
    ...
    def _track_reliable(self):
        ...
        if self.pad_conf <= 0.6:
            if not _flag("invest") or self.map_label == "forest":
                return False
            # weak but persistent track: go and look instead of dropping it
    ...
    def _cov_update(self, depth, pos, rpy):
        """Mark the 1 m cells of the search square whose surface was actually visible in this frame
        (back-projected depth: occluded ground behind houses / ridges stays unseen)."""
    ...
    def _cov_target(self, pos):
        """Centroid-ish cell of the densest unseen region (within 18 m of the centre), or None."""
        if self.cov_seen is None:
            return None
    ...
    def _set_mode(self, m):
        if m != self.mode:
            if m == "APPROACH":
                # remember whether this approach started from a weak (low-confidence) candidate
    ...
    def _plan(self, depth, pos, rpy, v_des, tube, tube_min, lookahead=10.0):
        ...
        if _flag("brake") and self.map_label in ("city", "mountain"):
            # the steep-climb candidates lie outside the image: they are not free, they are unknown.
            # Bound them by whatever the top rows show (overhangs, roofs of the building we hug).
        ...
        if _flag("brake") and self.map_label in ("city", "mountain"):
            ...
            ang_w = 0.4  # a wall straight ahead loses to a free diagonal earlier
        ...
        if _flag("brake") and self.map_label in ("city", "mountain") and self.planner.cy_raw[best] > 1.0:
            out_speed = min(out_speed, 1.0)  # climbing blind: crawl
        # braking: speed <= sqrt(2 a (f - margin))
        margin = 0.6
        decel = 2.2
        if _flag("brake") and self.map_label in ("city", "mountain"):
            margin, decel = 1.2, 1.5  # the velocity loop lags: 2.2 m/s2 from 2.9 m/s ran into walls
    ...
    def act(self, observation):
        ...
        if _flag("gndfilt") and self.map_label == "forest":
            # the downward ray hits tree crowns / roofs: take the lowest surface seen in the last 4 s as
            # the ground so a canopy below does not make us climb into the next one
        ...
        if _flag("vcruise12") and self.map_label == "village":
            h_cruise = 12.0  # arrive high: the line of sight to the search square clears the houses
        ...
        if self.mode == "TAKEOFF":
            ...
            if _flag("whfast") and self.map_label == "warehouse":
                target_h = min(target_h, 3.5)  # the king lands warehouse seeds in ~15 s; a 6.5 m climb alone costs 4 s
            # do not climb into a low ceiling: top rows of the depth image
            top = DEPTH_MIN_M + depth[0:12, 32:96].min() * (DEPTH_MAX_M - DEPTH_MIN_M)
            # forest: nothing observes the direction of travel during takeoff - the ceiling
            # guard reads the top rows of a forward camera, which look up at 40-45 deg,
            # never straight up - so react from further out and climb slowly enough to stop
            _ceil = 6.0 if self.map_label == "forest" else 2.5
            ready = alt >= target_h or climbed >= target_h + 0.5 or (top < _ceil and climbed > 0.8)
            ...
            vz = float(np.clip(1.2 * (h_cruise - alt_c), -2.5 if alt_c > 8.0 else -1.0, 1.5))
            ...
            if self.map_label == "mountain" and d_h < 25.0:
                ...
                if _flag("mhigharr"):
                    z_t = max(float(center[2]) + 8.0, pos[2] - alt + 2.0)  # look into the hollows from above first
            ...
            if self._track_reliable():
                ...
                if self.search_wps is None:
                    ...
                    if _flag("ringback") and self.map_label != "mountain":
                        # we arrived looking ahead: the half-disk behind us is the unseen one, ring it first
                        a0 = self.spin_start_yaw + math.pi
                    for r in radii:
                        for k in range(n_wp):
                            a = a0 + k * step
                            wps.append(c[:2] + r * np.array([math.cos(a), math.sin(a)]))
                    if _flag("nospin") and self.map_label != "mountain":
                        self.spin_done = True
                    if getattr(self, "hinted", False):
                        self.spin_done = True
                        self.hinted = False  # mountain keeps the centre spin: its vantage found pads the rings missed
                    self.search_wps = wps
                    self.search_i = 0
                    self.n_wp_ring = n_wp
                ...
                if self.map_label == "village" and (_flag("vhigh") or _flag("vhigh2")):
                    h_s = 9.5  # look over the houses: the line of sight to a ground pad 10+ m away clears 5 m roofs
                ...
                if self.map_label == "mountain":
                    ...
                    if _flag("mspin2") and not self.spin_done:
                        # look around from lower than the (noisy) centre height: hollow pads need a steep view
                        z_t = max(float(center[2]) - 1.5, pos[2] - alt + 2.0)
                    if _flag("mlow"):
                        # terrain-following search: a small blind cone (radius ~ height) beats a fixed height
                        z_t = pos[2] - alt + 2.8
                    vz = float(np.clip(1.2 * (z_t - pos[2]), -2.5 if alt > 8.0 else -1.0, 1.5))
                if not self.spin_done:
                    ...
                    if self.spin_acc > spin_need or (self.t - self.mode_t0) > spin_cap:
                        self.spin_done = True
                else:
                        ...
                        if self.cov_wp is None or self.t - self.cov_t_plan > 2.5 or float(np.linalg.norm(self.cov_wp - pos[:2])) < 2.5:
                            ...
                            if tgt is not None:
                                ...
                                if float(np.linalg.norm(pos[:2] - tgt)) < 1e-3:
                                    away = np.array([math.cos(yaw), math.sin(yaw)])
                                self.cov_tgt = tgt
                                self.cov_wp = tgt + away * 7.0  # a vantage 7 m short of the unseen patch
                    ...
                    dir_h = unit(to)
                    v_srch = prm["v_search"]
                    if _flag("vsearch3") and self.map_label == "mountain":
                        v_srch = 3.0  # the search budget after a 30 s cruise is short: fly the rings at the cap
                    ...
                    if cov_look is not None:
                        ...
                        if float(np.linalg.norm(lk)) > 1.0:
                            ...
                            if d_h < 1.5:
                                v_des = np.array([0.0, 0.0, vz])  # at the vantage: hold and look
                    if (self.map_label == "village" or (self.map_label == "mountain" and _flag("inward_mtn"))) and self.search_center_xy is not None and cov_look is None:
                        ...
                        if float(np.linalg.norm(inward)) > 2.0:
                            ...
                            if _flag("ringout") and self.map_label == "village" and self.search_i < getattr(self, "n_wp_ring", 4):
                                # depth saturates at 20 m: the outer band (15-24 m from the centre) is only
                                # visible from the first ring looking outward; the inner disk was seen on arrival
                                off = -off
            ...
            if pad is None:
                ...
                lost = self.pad_miss_t > 1.2 or unseen > 3.0  # a weak candidate gets a short look only
            ...
            if lost:
                if weak:
                    # false alarm: resume the search pattern where it was, do not re-anchor on the ghost;
                    # if the search has not started yet and we are still far from the centre, keep cruising
                    self._drop_track()
                    ...
                    if _flag("mreacq") and self.map_label == "mountain":
                        up_ = 2.5  # 5 m back / 4 m up puts a hollow pad at -39 deg: at the edge of the camera cone
                ...
                if self.map_label == "forest":
                    ...
                    if _flag("apptube") and self.map_label == "city":
                        tube = min(prm["tube"], 0.9)  # fast approaches brushed roofs and walls with a 0.6 m tube
                        tube_min = 0.4
                ...
                if not self.pad_moving:
                    ...
                    if d_h > 4.0:
                        ...
                        if (dt_h < 0.6 and vmatch < 0.8 and h < 1.6
                            ...
                            if _flag("gentle2"):
                                # only pads that have been slow for a while: a linear pad pausing at its
                                # turning point must stay a mover
                                demote = sp < 0.25 and self.pad_fit_sp < 0.25 and self.pad_slow_t > 1.5
                            if demote:
                                self.pad_moving = False  # never really moved: land it as a static pad
                                self.pad_vel = np.zeros(2)
                            self._set_mode("LAND")
                            self.land_z = float(pad[2])
                            self.land_t0 = self.t
            ...
            if self.reacq_moving and self.pad_pos is not None:
                ...
                if _flag("mmodel") and self.mm is not None and self.t - self.mm["t0"] < 12.0:
                    # the motion model still knows where the pad is: look there, not at the orbit centre
                    pxy, _ = self._mm_predict(self.t + 1.0)
                ...
                if _flag("reacqorbit") and self.t - self.reacq_t_enter > 4.0:
                    # still nothing from the vantage point: circle the orbit centre looking inward
            ...
            if self.touch_t > 0.0:
                ...
                if d_xy > 0.35:
                    ...
                    # normally a full stop to centre precisely, because the ground beside
                    # the pad is higher than its top - but with the horizon closing, the
                    # residual mountain failures ran out of clock inside this stop
                    vz = -0.25 if self.t > 50.0 else 0.0
                else:
                    vz = ((-0.60 if h > 0.25 else -0.50) if self.t > 50.0
                ...
                if _flag("mmodel") and not _flag("noamb") and self.mm is not None and self.t - self.mm["t0"] < 12.0:
                    # turning-point ambush: the fitted model says when and where the pad (nearly) stops
                    ...
                    if prev_ts is not None and prev_ts - self.t > 0.3:
                        # keep the committed stop while it is still ahead (no target hopping)
                ...
                if amb is not None:
                    ...
                    if dt_star > 1.3 or d_s > 0.4:
                        vz = float(np.clip(1.5 * (pad[2] + 0.9 - pos[2]), -1.0, 1.0))  # wait 0.9 m above the stop point
                    else:
                        vz = -1.0  # the pad arrives in ~1 s: descend onto it
                    ...
                    self.land_phase = "track"  # model gone: back to pursuit
                if amb is None and self.land_phase == "track":
                    if _flag("trackview"):
                        # keep the pad inside the forward camera's view (it sits ~40 deg below the horizon from
                        # here): a little higher and further behind than the old 1.0 m / 1.3 h geometry, and
                        # when it drops out of view climb instead of hovering over a stale estimate
                    ...
                    if _flag("softdrop") and sp < 0.6:
                        # a mis-flagged static pad (false speed 0.3-0.6 m/s) tips the drone when slammed at
                        # 1 m/s; fast movers keep the quick drop (a slow last metre lets them escape)
                        vz = max(vz, -0.7 if h < 1.0 else -1.1)
                        if h < 0.45:
                            vz = max(vz, -0.5)
                    if (_flag("gentle") and sp < 0.35) or (_flag("gentle2") and sp < 0.35 and self.pad_slow_t > 1.0):
                        # a pad this slow is probably static and mis-flagged: touch down like a static one
                        # (a 1 m/s slam on a sloped mountain pad tips the drone over), centred first
                ...
                if _flag("mvpatience") and d_xy > 0.3:
                    # the pad slid past us: the camera needs |yaw error| / 0.69 rad/s to swing back onto it
                    unseen_lim += min(4.0, abs(wrap(yaw_des - yaw)) / 0.69)
                if pos[2] < pad[2] - 0.45 or (unseen > unseen_lim and h > 0.7) or self.t - self.land_t0 > (22.0 if amb is not None else 20.0) or self.land_retry > 2:
                    abort = True
            if abort:
        ...
        if use_planner:
            ...
            if _flag("yawvel") and self.map_label in ("city", "mountain") and self.mode in ("CRUISE", "SEARCH", "REACQUIRE"):
                # look where we fly: after a detour the camera must not stay on the goal while the
                # drone runs sideways into terrain it cannot see (yaw slews at 0.69 rad/s only)
                vh = float(np.hypot(v_des[0], v_des[1]))
                if vh > 0.8:
                    ang_v = wrap(math.atan2(v_des[1], v_des[0]) - yaw_des)
                    if abs(ang_v) > 0.35:
                        yaw_des = math.atan2(v_des[1], v_des[0])
                    # the actual velocity is what carries us into the unseen: cap it while off-camera
        ...
        if (_flag("roofguard") and self.map_label == "city" and v_des[2] < 0.0
                and self.mode in ("CRUISE", "APPROACH", "SEARCH", "REACQUIRE")):
            # descending over a roof/wall that sits just below-beside the flight line: the tube test does
            # not see it. Hold altitude while anything in the lower image band is within 3 m, unless we
            # are already over the pad column.
...
_KING_SRC_B64 = (
    ...
    "EHDUcwx9eWJHxtcyyPAzkMv1zusTqOb62GkudO81O7Z2bPXo6gK29UT5fipEZDnQK+sVPnCdZms8yUfi4KfPaCvdVTp7Jc22mmvi"
    "i7zecBHbsi0yX4GBr6z3XirggdtpLznAY3aPdyUCDcWLyZu0qy9FphOagFq+EwXB4Fp/PyMBDE0B0WpxmQ1xLMzLNxg82xSmDa6K"
    "cg4CYj9cBYq6LwWFJOXd1+IEykENYCrzdL9yiDWwG7/kaqRkgM8qkV2urlwkWBhUx1pkV3hJLxq3FF45T3ByfCctGZxvXf3Nv6NS"
    "wHjD+hIcJI0AdQl4ZZUF4RDRvaiCXqPjLe90LK/rxpD04WrWNqiUyZRBVQx/SuuStH0astLXmzI9x1ML8b/JRtc55gH6W/r37V8g"
    "jaz9EDV0s93pRkugdBzgzBEeGzfnWhBYxRoCLyBGawtzV2ocnbXZBjYJs1WuLVUqARYlT7VJwoN6G4KCBfzGbHOTQcygVa4jXru5"
    "+FwrJvQ46elyBWNI2Bg0ohYq5df4CQTxrCGwI6PtpRe05pjPYmaAybivwCLgsWPr8ZDLe5TwleaAsSB/MeHDV8CYKSX1nOGQvrxW"
    "Z9pgHFOB7JvmmpPuo0YwDYIN58DptsyV+CPmYHis7K+VjIzpFOX5eGuovqxWbU39UQl4AcqNUCSQkfdKkgeN6K92H8Jd9NcSmJnL"
    "wWY/JHJzJiTbVYCOlynwdwUvCAUlmVogA4OwZwnc/I+mjEjrvFwiv3ww4mgCh3ngOr5MPA3WKywzq2TBzmkQW/pLOMnk0MEkFU+d"
    "plIqJyU896Jgea6ru3Ga35CHZiO489uo1DMxG0Jfns3Madg9sUXPalofxnI1DdN3JNLvN2h/XjeNMo9fpfeKhLjO8VAAD20eBNyZ"
    "ylbgSAOlzacOzsskifLRe5g8Uj241SQb0ep5EueqqkTAASywpMIVV6mH2GF31HgLDzsXeIdzoHrOPFC6/ihRmbkQJ2ZfLpi2rrRE"
    "KGDGU1+BqPGsAqbrHO5/7Nh5BB+TeF1AHKpvwBKO42vXOJQuLeaoEF9zgle1c1oQEkBNEbyGCPZ+231kPoGqcsnUTcr0mbRjoj1J"
    "FVBKwIRjRDaBSECz2PYNwNSpED4LYXxFKvqWYHgaT9svvQvp+P2vp9Nu/D/J/QfD"
)

_HANDOVER = os.environ.get("MY_HANDOVER", "any")  # mountain: hand any moving pad to the king (fast-only handover lost 3 seeds on P1)
...
_NO_MOTION_HO = tuple(x for x in os.environ.get("MY_NOMOTION_HO", "").split(",") if x)  # labels whose moving pads stay with my controller
...
_KING_MOVING_LABELS = (("open",) if os.environ.get("MY_ROUTE_OPEN", "king") == "dual" else ()) + (("city",) if os.environ.get("MY_ROUTE_CITY", "dual") != "king" else ()) + (("mountain",) if os.environ.get("MY_ROUTE_MTN", "dual") == "dual" else ()) + (("village",) if os.environ.get("MY_ROUTE_VIL", "dual") == "dual" else ())  # both fly; the king takes over once the pad proves to be moving
...
class DroneFlightController:
    ...
    def act(self, observation):
        # the validator re-calls act() with the SAME observation after a slow step; my controllers are
        # not idempotent (step clock, tracker), so an identical observation gets the identical action
        try:
    ...
    def _act_inner(self, observation):
        ...
        if self.route == "dual":
            ...
            if _flag("kingsee") and a_king is not None and self.mine.pad_pos is None and self.mine.mode in ("SEARCH", "REACQUIRE"):
                # my search has nothing but the shadow-flying king already sees the pad: let it land
                try:
                    for obj in (getattr(self.king, "_main", None), getattr(getattr(getattr(self.king, "_uid130", None), "_base", None), "_main", None)):
            ...
            if (_flag("mtnfb") and a_king is not None and self.mine.map_label in _FB_LABELS and self.mine.t > _MTNFB_T
                ...
                # late without a pad: the shadow-flying king takes over (its own search from here)
                self.route = "king"
                self.debug = {"route": "king", "map": self.mine.map_label, "mode": "KING", "pad_hits": 0}
                return a_king
            ...
            if fast_ok and (_HANDOVER_CITY if self.mine.map_label == "city" else _HANDOVER) == "fast":
                # hand over only pads that really move fast; slow ones land better with my track/drop logic
