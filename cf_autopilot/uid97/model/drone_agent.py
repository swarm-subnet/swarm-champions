...
class _MyController:
    ...
    def act(self, observation):
        ...
        if self.mode == "TAKEOFF":
            ...
            v_des = np.array([0.0, 0.0, (2.5 if (open_sky or (_flag("whfast") and self.map_label == "warehouse")) else 1.5) if not ready else 0.0])
            use_planner = False
...
_KING_MOVING_LABELS = (("open",) if os.environ.get("MY_ROUTE_OPEN", "king") == "dual" else ()) + (("city",) if os.environ.get("MY_ROUTE_CITY", "dual") != "king" else ()) + (("mountain",) if os.environ.get("MY_ROUTE_MTN", "dual") == "dual" else ()) + (("village",) if os.environ.get("MY_ROUTE_VIL", "mine") == "dual" else ())  # both fly; the king takes over once the pad proves to be moving
